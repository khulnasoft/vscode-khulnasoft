...
namespace StatusNotification {
    export const type = new NotificationType<StatusParams, void>('khulnasoft/status');
}
...
const exitCalled = new NotificationType<[number, string], void>('khulnasoft/exitCalled');
...
let khulnasoftServer: string = undefined;
...
connection.onInitialize((params) => {
    ...
    khulnasoftServer = getServerUrl(initOptions.server);
    ...
    connection.console.info(`Server: ${khulnasoftServer} (${userAgent})`);
    return {
        capabilities: {
...
connection.onDidChangeConfiguration((params) => {
    ...
    if (!_.isEqual(khulnasoftServer, settings.khulnasoft.server)) {
        khulnasoftServer = getServerUrl(settings.khulnasoft.server);
        changed = true;
    }

    if (!_.isEqual(proxyServer, settings.khulnasoft.proxy)) {
        proxyServer = settings.khulnasoft.proxy;
        changed = true;
    }

    if (!_.isEqual(fileSuffixes, settings.khulnasoft.fileSuffixes)) {
        initializeSupportedFileSuffixes();
        changed = true;
    }

    if (changed) {
        connection.console.info(`Configuration changed: ${khulnasoftServer} (proxy: ${proxyServer}, fileSuffixes: ${fileSuffixes})`);
    }
});

connection.onExecuteCommand((e: ExecuteCommandParams) => {
    if (e.command === 'khulnasoft.updateToken') {
        token = e.arguments[0];
    }
});

connection.onRequest('khulnasoft.getTokenInfo', async () => {
    try {
        const apiPath = `${khulnasoftServer}/api/vscode/tokeninfo`;
...
connection.onCompletion((_textDocumentPosition: TextDocumentPositionParams): CompletionItem[] => {
    return [{
        label: 'khulnasoft-disable',
        kind: CompletionItemKind.Text,
        data: 1
    }, {
        label: 'khulnasoft-enable',
        kind: CompletionItemKind.Text,
        data: 2
    }, {
        label: 'khulnasoft-disable-line',
        kind: CompletionItemKind.Text,
        data: 3
    }, {
        label: 'khulnasoft-disable-next-line',
        kind: CompletionItemKind.Text,
        data: 4
    }];
});
...
connection.onCompletionResolve((item: CompletionItem): CompletionItem => {
    switch (item.data) {
        case 1:
            item.detail = 'KhulnaSoft directives';
...
async function inspect(identifier: VersionedTextDocumentIdentifier) {
    const guideUrl = `${khulnasoftServer}/docs/khulnasoft/vscode#token`;
    const generateUrl = `${khulnasoftServer}/dashboard/#view=account-settings`;

    let uri = identifier.uri;
    let textDocument = documents.get(uri);
    let docContent = textDocument.getText();

    const URL = `${khulnasoftServer}/api/vscode/analysis`;
    ...
    if (!token) {
        const message = `Failed to inspect: KhulnaSoft access token not configured. Visit ${guideUrl} to generate a token.`;
    ...
    if (Array.isArray(settings.khulnasoft.ignorePatterns)) {
        const ig = ignore().add(settings.khulnasoft.ignorePatterns);
        if (ig.ignores(uri)) {
            sendDiagnostics([]);
            return;
        }
    ...
    try {
        ...
        if (Array.isArray(settings.khulnasoft.ignoreRules)) {
            diagnostics = diagnostics.filter(diagnostic =>
                !settings.khulnasoft.ignoreRules.includes(diagnostic.code as string)
            );
        }
        ...
        if (isTokenMessage && message.includes('expired')) {
            ...
            message = `KhulnaSoft access token has expired. Visit ${generateUrl} to regenerate the token.`;
        } else if (isTokenMessage && message.includes('Invalid')) {
            state = Status.INVALID_TOKEN;
            message = `invalid KhulnaSoft access token. Visit ${generateUrl} to regenerate the token and make sure to copy the correct token string.`;
        }
