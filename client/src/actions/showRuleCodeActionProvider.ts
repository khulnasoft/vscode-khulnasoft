...
export default class showRuleCodeActionProvider extends khulnasoftCodeActionProvider {
  ...
  public execute(document, ruleKey: string) {
    ...
    if (!this._panel) {
      const viewType = "khulnasoft.show-rule";
      const tabTitle = "KhulnaSoft Rule";
  ...
  implements vscode.TextDocumentContentProvider
{
  ...
  private createSnippet() {
    ...
    if (
      this.rules &&
      (rule = _.find(this.rules, (rule) => rule.key === this.ruleKey))
    ) {
      ...
      content = `<ul class="khulnasoft-rule-detail">
                        <li class="khulnasoft-rule-detail-property">`;
      _.forEach(rule.severity, (severity) => {
        content += `<span class="severity" data-severity="${severity}"><i class="circle"></i>${severity}</span>`;
      });
      content += `<li class="khulnasoft-rule-detail-property"><span class="icon icon-${
        rule.type === "Error" ? "error" : "code-quality"
      }"></span> ${rule.type}
                        <li class="khulnasoft-rule-detail-property"><span class="icon icon-tags"></span> ${
                          tags.length > 0 ? tags.join(", ") : "No tags"
                        }
                        <li class="khulnasoft-rule-detail-property"><span class="icon icon-bookmark"></span> <a href="https://khulnasoft.com/docs/rules/${slugify(
                          rule.key
                        )}">${rule.key}</a>
                       </ul>

                       <div class="khulnasoft-rule-description">
                           <h2>${rule.name}</h2>
                           ${this.converter.makeHtml(rule.description)}
                           <h3>Code Example</h3>
                           <pre><code class="language-javascript">${_.escape(
                             rule.examples
                           )}</code></pre>
                           <h3>Revised Code Example</h3>
                           <pre><code class="language-javascript">${_.escape(
                             rule.examplesRevised
                           )}</code></pre>`;
      if (sees.length > 0) {
        content += `<h3>See</h3>
                            <ul>`;
    ...
    return `<style>${this.style}</style><body><div class="khulnasoft-rule">${content}</div></body>`;
  }
}
