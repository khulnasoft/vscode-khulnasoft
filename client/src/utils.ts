...
export function updateTokenRequest(client: LanguageClient, newToken: string) {
    const params: ExecuteCommandParams = {
        command: 'khulnasoft.updateToken',
        arguments: [ newToken ]
    };

    const successCallback = () => {
        if (newToken) {
            client.info('New KhulnaSoft access token was configured.');
        } else {
            client.info('KhulnaSoft access token was deleted.');
        }
    };
    client.sendRequest(ExecuteCommandRequest.type, params).then(successCallback,
        (error) => {
            console.error('Server failed', error);
            vscode.window.showErrorMessage('Failed to send a request for updating KhulnaSoft access token. Please consider opening an issue with steps to reproduce.');
        }
    );
}
